...
func TestAccAzureRMKubernetesCluster_addonProfileAciConnectorLinux(t *testing.T) {
	...
	config2 := testAccAzureRMKubernetesCluster_addonProfileAciConnectorLinux2(ri, clientId, clientSecret, testLocation())
	config3 := testAccAzureRMKubernetesCluster_addonProfileAciConnectorLinux3(ri, clientId, clientSecret, testLocation())
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "addon_profile.#", "1"),
				),
			},
			{
				Config: config2,
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMKubernetesClusterExists(resourceName),
					resource.TestCheckResourceAttr(resourceName, "addon_profile.#", "0"),
				),
			},
		},
	})
...
func testAccAzureRMKubernetesCluster_addonProfileAciConnectorLinux2(rInt int, clientId string, clientSecret string, location string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-%d"
  location = "%s"
...
func testAccAzureRMKubernetesCluster_addonProfileAciConnectorLinux3(rInt int, clientId string, clientSecret string, location string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-%d"
  location = "%s"
