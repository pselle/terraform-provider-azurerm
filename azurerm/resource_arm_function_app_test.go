...
func TestAccAzureRMFunctionApp_removeSiteConfig(t *testing.T) {
	resourceName := "azurerm_function_app.test"
	ri := tf.AccRandTimeInt()
	rs := strings.ToLower(acctest.RandString(11))
	config := testAccAzureRMFunctionApp_alwaysOn(ri, rs, testLocation())
	noBlocksConfig := testAccAzureRMFunctionApp_noBlock(ri, rs, testLocation())
	blocksEmptyConfig := testAccAzureRMFunctionApp_emptyBlock(ri, rs, testLocation())
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testCheckAzureRMFunctionAppExists(resourceName),
					resource.TestCheckResourceAttr(resourceName, "site_config.#", "1"),
				),
			},
			{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "site_config.#", "0"),
				),
			},
		},
	})
...
func testAccAzureRMFunctionApp_noBlock(rInt int, rString, location string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-%[1]d"
  location = "%[2]s"
...
func testAccAzureRMFunctionApp_emptyBlock(rInt int, rString, location string) string {
	return fmt.Sprintf(`
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-%[1]d"
  location = "%[2]s"
